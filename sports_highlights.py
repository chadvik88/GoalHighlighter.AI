...
highlight_objects = {'sports ball', 'ball'}  # Checking for ball detection
AUDIO_THRESHOLD = 0.6  # Threshold for detecting loud crowd cheers
HIGHLIGHT_DURATION = 5  # Clip duration (seconds)

# Video file input
input_video = "match.mp4"

# --- STEP 1: DETECT GOALS USING OBJECT DETECTION & OCR ---
def detect_goal_frames(video_path):
    ...
    while cap.isOpened():
        ...
        # Check for sports ball
        if any(obj in highlight_objects for obj in detected_classes):
            # Extract scoreboard text using OCR
            gray_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
            detected_text = reader.readtext(gray_frame, detail=0)
            print(f"Detected text at frame {frame_count}: {detected_text}")  # Debug print

            # Extract numbers from OCR
            numbers = [int(s) for s in detected_text if s.isdigit()]
            if len(numbers) >= 2:  # Assuming two numbers represent scores
                score = tuple(numbers[:2])  # Take the first two numbers
                if score != last_score:
                    last_score = score
...
# --- STEP 3: ANALYZE AUDIO FOR CROWD CHEERS ---
def analyze_audio(audio_path):
    y, sr = librosa.load(audio_path, sr=None)
    rms = librosa.feature.rms(y=y)[0]
    cheer_spikes = np.where(rms > AUDIO_THRESHOLD)[0]
    
    print("Detected audio spikes:", cheer_spikes)  # Debug print
    return cheer_spikes

# --- STEP 4: CONFIRM GOALS BASED ON AUDIO SPIKES ---
def confirm_goals(goal_frames, audio_spikes, fps):
    ...
    for goal_time in goal_timestamps:
        if any(abs(goal_time - (spike / fps)) < 2 for spike in audio_spikes):  # Within 2 seconds
            confirmed_goals.append(goal_time)

    return confirmed_goals

# --- STEP 5: EXTRACT HIGHLIGHTS USING FFMPEG ---
...
# --- STEP 6: MERGE HIGHLIGHTS INTO FINAL VIDEO ---
